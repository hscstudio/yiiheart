 ...
 *
 * The followings are the available columns in table 'tb_training':
 * @property integer $id
...
class Training extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name', 'required'),
			array('status, createdBy, modifiedBy, deletedBy', 'numerical', 'integerOnly'=>true),
			array('name, note', 'length', 'max'=>255),
			array('start, finish, created, modified, deleted', 'safe'),
			/*
			//Contoh username
			...
			array('id, name, start, finish, note, status, created, createdBy, modified, modifiedBy, deleted, deletedBy', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'trainingDocuments' => array(self::HAS_MANY, 'TrainingDocument', 'tb_training_id'),
		);
	}
